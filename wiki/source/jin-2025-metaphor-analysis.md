---
title: "Jin et al. (2025) — High heels, compass, spider-man, or drug? Metaphor analysis of generative artificial intelligence in academic writing"
aliases:
  - "Jin et al. (2025)"
type: source
themes:
  - edtech
  - sociocultural
position: descriptive
proponents:
  - [[Fangzhou Jin]]
  - [[Lanfang Sun]]
  - [[Yunqiu Pan]]
  - [[Chin-Hsi Lin]]
status: active
draft: false
created: 2026-06-02
updated: 2026-06-02
sources:
  - jin-2025
---

Peer-reviewed empirical article in *Computers & Education* (Elsevier, 2025). Fangzhou Jin and colleagues at the University of Hong Kong report a metaphor analysis study of N=277 international postgraduate students from 14 countries studying at 18 UK universities. The study identifies 53 unique metaphors for GenAI in academic writing and organises them into a four-category taxonomy called the 4T Pyramid Model.

## Abstract

> This research employed metaphor analysis to explore 277 postgraduate students' perceptions of the role of generative artificial intelligence (GenAI) in academic writing. All participants were international students, from a total of 14 countries and regions, studying in the United Kingdom. Data collection was carried out in two phases. The first was a survey comprising demographic and metaphor-related questions, and the second involved metaphor checking, in which participants provided screenshots of their interactions with GenAI. The data, which were analyzed both qualitatively and quantitatively, yielded 53 unique metaphors for the concept of GenAI in academic writing. We divided these into four conceptual categories in what we term the 4T Pyramid Model: Technical Support (representative metaphor: high-heeled shoes), Text Development (compass), Transformative Potential (Spider-Man), and Threat (drug). The respondents' academic disciplines influenced their perceptions of GenAI, but overall, the results suggest that most viewed it as transformative, i.e., more than just a writing tool. This study's innovative methodology integrating metaphor analysis with real user interactions offers a framework, aligned with Bloom's Taxonomy, that reveals the multi-level benefits and potential risks of GenAI. It also provides actionable insights for AI literacy education, including strategies for effective prompt design.

**DOI:** https://doi.org/10.1016/j.compedu.2025.105248

## Summary

The study addresses two gaps in the literature: most research on GenAI in academic writing has used positivist experimental methods and drawn on single cultural backgrounds, producing limited insight into the diverse, discipline-specific perceptions that international students — who face unique academic writing challenges — bring to GenAI tools. Jin et al. argue that metaphor analysis can reveal the full range of positive and negative perceptions, including affective and implicit dimensions that surveys and experiments do not capture.

Data collection proceeded in two phases. First, 277 international postgraduate students at 18 UK universities completed a demographic and metaphor-elicitation survey. Second, a subsample provided screenshots of their actual GenAI interactions, enabling validation of the metaphors against real use patterns. Qualitative and quantitative analysis of the 53 metaphors yielded four conceptual categories, grouped into what the authors call the 4T Pyramid Model: Technical Support (exemplary metaphor: high-heeled shoes — functional assistance without which certain tasks become difficult); Text Development (compass — guiding the writing process toward clarity and direction); Transformative Potential (Spider-Man — ability to produce effects beyond ordinary human capacity); and Threat (drug — dependency, loss of original voice, ethical risk).

Most students, across disciplines, conceptualised GenAI as primarily Transformative rather than merely a writing tool. Disciplinary background significantly influenced metaphor choice: students in social sciences and humanities showed more ambivalence and were more likely to invoke threat metaphors, while students in STEM and business disciplines showed stronger Technical Support framings. The study contributes a cross-disciplinary, cross-cultural empirical basis for designing AI literacy education that addresses the threat dimension alongside the transformative.

## Entities extracted

- **Concepts:** [[4T Pyramid Model]]; metaphor analysis in AI research; cross-cultural AI perception; AI literacy education
- **Actors:** [[Fangzhou Jin]] (University of Hong Kong); [[Chin-Hsi Lin]] (University of Hong Kong)
- **Themes:** disciplinary variation in GenAI perception; transformative vs. threat framings of AI

## Cross-source links

- [[Bearman et al. (2023)]] — both examine how AI is discursively framed in educational contexts; Bearman uses academic literature as data; Jin et al. use student metaphors
- [[Tully et al. (2024)]] — both examine perception of AI in non-expert populations; Tully finds literacy moderates perception of AI as magical; Jin et al. find disciplinary background shapes metaphor choice
- [[Conradty et al. (2026)]] — both examine student conceptions of AI; Conradty in European secondary school context; Jin et al. in UK postgraduate context; both find students maintain normative boundaries around AI use

## Citation

Jin, F., Sun, L., Pan, Y., & Lin, C.-H. (2025). High heels, compass, spider-man, or drug? Metaphor analysis of generative artificial intelligence in academic writing. *Computers & Education*, *228*, 105248. https://doi.org/10.1016/j.compedu.2025.105248
