---
title: "Tully et al. (2024) — Lower Artificial Intelligence Literacy Predicts Greater AI Receptivity"
aliases:
  - "Tully et al. (2024)"
type: source
themes:
  - sociocultural
  - hype
position: descriptive
proponents:
  - [[Stephanie Tully]]
  - [[Chiara Longoni]]
  - [[Gil Appel]]
status: active
draft: false
created: 2026-06-02
updated: 2026-06-02
sources:
  - tully-2024
---

Peer-reviewed empirical article in the *Journal of Marketing* (SAGE/AMA, 2025 volume, submitted 2024). Stephanie Tully (USC Marshall School), Chiara Longoni (Bocconi University), and Gil Appel (GWU) report ten studies — four surveys, cross-country data, and six additional studies — demonstrating a counter-intuitive negative relationship between objective AI literacy and AI receptivity, mediated by perception of AI as magical.

## Abstract

> As artificial intelligence (AI) transforms society, understanding factors that influence AI receptivity is increasingly important. The current research investigates which types of consumers have greater AI receptivity. Contrary to expectations revealed in four surveys, cross-country data and six additional studies find that people with lower AI literacy are typically more receptive to AI. This lower literacy–greater receptivity link is not explained by differences in perceptions of AI's capability, ethicality, or feared impact on humanity. Instead, this link occurs because people with lower AI literacy are more likely to perceive AI as magical and experience feelings of awe in the face of AI's execution of tasks that seem to require uniquely human attributes. In line with this theorizing, the lower literacy–higher receptivity link is mediated by perceptions of AI as magical and is moderated among tasks not assumed to require distinctly human attributes. These findings suggest that companies may benefit from shifting their marketing efforts and product development toward consumers with lower AI literacy. In addition, efforts to demystify AI may inadvertently reduce its appeal.

**DOI:** https://doi.org/10.1177/00222429251314491

## Summary

The prevailing assumption in AI adoption research and marketing practice is that greater AI literacy leads to greater receptivity: better understanding should produce more positive attitudes and more willingness to use AI products. Tully et al. directly challenge this assumption through a multi-study program of research. They find that consumers with lower objective AI literacy exhibit greater AI receptivity — a relationship they term the "lower literacy-higher receptivity link." Importantly, this link is not explained by differences in perceptions of AI's capability, ethicality, or feared impact on humanity.

The mechanism Tully et al. identify is perceptual: people with lower AI literacy are more likely to perceive AI systems as magical, and to experience feelings of awe when AI performs tasks they associate with distinctively human attributes (such as having a personality, feeling emotions, or holding beliefs). AI systems executing such tasks appear to transcend the boundaries of conventional technology for lower-literacy observers. In contrast, consumers with higher AI literacy recognise that AI does not actually possess human attributes when executing these tasks, thereby reducing perceptions of magicality and dampening awe — and with it, receptivity. The literacy-receptivity link is moderated by task type: it attenuates for tasks not associated with distinctively human attributes.

The practical implication is counterintuitive for marketers: AI literacy education, typically advocated as a route to informed adoption, may inadvertently reduce the appeal of AI products by demystifying them. The authors suggest that efforts to "demystify AI" may reduce receptivity among currently receptive lower-literacy consumers while not necessarily converting higher-literacy skeptics.

## Entities extracted

- **Constructs:** [[AI Literacy]] (objective measure); [[AI Receptivity]] (candidate); awe; perceived magicality
- **Concepts:** lower literacy-higher receptivity link; AI anthropomorphism; AI as magical technology
- **Actors:** [[Stephanie Tully]] (USC Marshall); [[Chiara Longoni]] (Bocconi University); [[Gil Appel]] (George Washington University)
- **Claims:** [[Lower AI Literacy Predicts Greater AI Receptivity]]
- **Themes:** consumer AI perception; hype and magical thinking about AI

## Cross-source links

- [[Xu and Xiong (2026)]] — **contested claim**: Xu & Xiong find AI literacy positively predicts actual GenAI usage; Tully et al. find objective literacy negatively predicts receptivity — constructs and populations differ; see `wiki/_review.md`
- [[Jin et al. (2025)]] — also examines non-expert AI perception; Jin et al. use metaphor analysis; Tully et al. use experimental surveys; both find diversity in how AI is perceived at the individual level
- [[Armstrong (2025)]] — also examines AI perception variation by demographic characteristics; Armstrong focuses on equity dimensions; Tully et al. on cognitive mechanism

## Citation

Tully, S., Longoni, C., & Appel, G. (2025). Lower artificial intelligence literacy predicts greater AI receptivity. *Journal of Marketing*, *89*(1). {{doi unverified — note: published 2025 in vol. 89; bib key uses 2024 (submission/working paper year)}}
